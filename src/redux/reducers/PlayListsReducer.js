...
const PlayListsReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case "ADD_MUSIC":
      ...
      return { ...state, playlists: tempPlaylist };
    case "EDIT_MUSIC":
      let tempTracks2 = tempPlaylist.filter((playlist) => playlist.id === action.playlistId)[0].tracks;
      tempTracks2.forEach((item) => {
        if (item.id === action?.musicUpdate?.id) {
          item.name = action?.musicUpdate?.name;
          item.preview_url = action?.musicUpdate?.preview_url;
          item.image = action?.musicUpdate?.image;
        }
      });
      tempPlaylist.forEach((item) => {
        if (item.id === action?.playlistId) {
          item.tracks.forEach((track) => {
            if (track.id === action?.musicUpdate?.id) {
              track.name = action?.musicUpdate?.name;
              track.preview_url = action?.musicUpdate?.preview_url;
              track.image = action?.musicUpdate?.image;
            }
          });
          return;
        }
